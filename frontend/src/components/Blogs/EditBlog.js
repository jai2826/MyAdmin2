...
const EditBlog = () => {
  ...
  const [categories, setCategories] = useState(selectedBlog.categories);
  ...
  const [post, setPost] = useState(selectedBlog.post);
  const postHandle = (e) => {
    setPost(e);
  };
  ...
  const SaveBlog = async (e) => {
    ...
    e.preventDefault();
    const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/blog/updateblog/${selectedBlog._id}`, {
      method: "PUT",
